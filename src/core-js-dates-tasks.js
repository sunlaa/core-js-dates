...
function getTime(date) {
  ...
  const seconds = normalize(date.getSeconds());
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return 1 + new Date(d2 - d1) / 1000 / 60 / 60 / 24;
}
...
function formatDate(date) {
  ...
  const m = normalize(d.getUTCMinutes());
  const s = normalize(d.getUTCSeconds());
...
function getWeekNumberByDate(date) {
  ...
  const shift =
    startYear.getUTCDay() === 1 ? 7 : (1 - startYear.getUTCDay() + 7) % 7;
  startYear.setUTCDate(startYear.getUTCDate() + shift);

  const shift2 = date.getDay() === 0 ? 6 : date.getDay() - 1;
  date.setUTCDate(date.getUTCDate() - shift2);
...
function getNextFridayThe13th(date) {
  while (!(date.getUTCDay() === 5 && date.getUTCDate() === 13)) {
    date.setUTCDate(date.getUTCDate() + 1);
  }
  return date;
}
...
function isLeapYear(date) {
  const february = new Date(date.getUTCFullYear(), 2, 1);
  february.setUTCDate(february.getUTCDate() - 1);

  if (february.getUTCDate() === 29) return true;
  return false;
}
